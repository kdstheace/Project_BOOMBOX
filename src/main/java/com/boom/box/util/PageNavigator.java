...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//(본인, 불변) 페이지당 글목록 수
	private int pagePerGroup;		//(본인, 불변) 그룹당 페이지 수  (아래 < << 12345 >> > 이 부분)
	private int currentPage;		// 현재 페이지 (최근 글이 1부터 시작)
	private int totalRecordsCount;	//(DB에서 가져옴)전체 글 수 - DB에서 셀렉문으로
	private int totalPageCount;		//전체 페이지 수 (전체 글 개수 + 몇개씩 보여줄지 - 1) /(몇개씩 보여줄지)
	private int currentGroup;		//(전달받음)현재 그룹 (최근 그룹이 0부터 시작) (>>이 버튼 누르면, 여기서는 4페이지에서 9페이지로) (13페이지까지 있을때는 11페이지에서 누르면 13페이지로감)
	private int startPageGroup;		//현재 그룹의 첫 페이지
	private int endPageGroup;		//현재 그룹의 마지막 페이지
	private int startRecord;		//전체 레코드 중 현재 페이지 첫 글의 위치 (0부터 시작 = 1페이지의 첫번째 글)
